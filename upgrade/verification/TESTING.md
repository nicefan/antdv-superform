...
## 2026-09-12 P010 回补：统一发布与 REPL 构建目录

- 修改：三个 npm 包的发布目录从 `lib/` 迁为 `dist/`；根目录 REPL 中间目录从 `dist/` 迁为 `.repl-dist/`。
- 覆盖：package exports、CSS sideEffects、Vite 与声明输出、REPL 同步路径、发布声明断言、忽略规则和文档。
- 未执行：按协作约束仅修改，未运行测试、类型检查或 build。发布前需构建生成本地 `dist/`；构建产物不提交到 Git。

## 2026-09-08 P003 回补：移除 customIcon

- 修改：删除全局 `customIcon`、`IconAdapterContext` 与 AntDV Adapter 的业务字符串图标回调；业务图标改为直接传入组件或节点。
- 文档：同步全局配置、AI 指引、设计说明和不兼容迁移记录。
- 未执行：按协作约束仅修改，未运行测试、类型检查或 build。
