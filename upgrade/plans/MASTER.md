...
状态：已完成并归档（2026-09-12）

后续改造独立记录在 [tasks/](../../tasks/README.md)，不再追加本计划。以下阶段进展为历史记录。
...
升级已结束。原阶段切换、任务完成即补文档和新增回补规则停止适用，统一执行根目录 `AGENTS.md` 与 `tasks/README.md` 的人工验收流程。历史任务和验收记录保留，不作为自动开启新工作的授权。
...
- [x] 删除只在 AntDV 生效的全局 `customIcon` 注册表；业务图标改为直接传入组件或节点。
...
#### 2026-09-08：回补移除 customIcon

- [x] 删除 `GlobalConfig.customIcon`、`IconAdapterContext` 及 AntDV Adapter 对全局字符串图标注册表的消费。
- [x] 保留 Adapter 语义图标和直接传入组件/节点的图标渲染；业务字符串不再经 SuperForm 映射。
- [x] 同步全局配置文档、AI 指引与不兼容迁移记录。
- 验证：按协作约束仅修改，未执行测试、类型检查或 build。
...
- [x] 完成真实消费项目的升级演练。
- [x] 将 npm 包构建产物目录统一为 `dist/`，文档 REPL 中间产物改为 `.repl-dist/`，避免根目录目录职责冲突。
...
#### 2026-09-12：回补统一发布与 REPL 构建目录

- 修改：Core 与两个官方 Adapter 的发布目录统一为 `dist/`；package exports、声明后处理、发布断言和公开文档同步更新。
- 修改：在线 REPL 的根目录临时 bundle 改为 `.repl-dist/`，同步脚本从该目录读取并写入 `docs/public/repl/`；原有 REPL bundle 已迁移到新目录。
- 修改：删除已不再维护的 `build:examples`，将 REPL 构建命令由 `build:dist` 改名为 `build:repl`。
- 验证：按协作约束仅修改，未执行测试、类型检查或 build；发布前需重新构建生成本地 `dist/`，但不提交构建产物。
