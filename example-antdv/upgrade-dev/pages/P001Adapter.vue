...
const switchAdapter = adapter.fields?.Switch
const resolveComponent = (name: string) => {
  const component = adapter.fields?.[name]?.component ?? name
...
const checks = computed(() => [
  ...
  {
    label: 'Input 组件解析',
    passed: resolveComponent('Input') === adapter.components.Input,
    value: resolveComponent('Input')?.name || '未解析',
  },
  {
    label: 'TimeRangePicker 真实名称',
    passed: resolveComponent('TimeRangePicker') === adapter.components.TimeRangePicker,
    value: resolveComponent('TimeRangePicker')?.name || '未解析',
  },
  {
