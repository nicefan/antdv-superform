...
const createLabelNode = (option, effectData) => {
  ...
  return _label === void 0 ? void 0 : () => [
    toNode$2(_label, effectData),
    tooltip && renderUIAction("tooltip", tipProps, {
      title: () => toNode$2(tooltip.title, effectData),
      default: () => h(
        "span",
        {
      ...
      transformProps(props) {
        ...
        return {
          ...
          options: optionsRef.value.map((item) => ({
            ...item,
            label: toNode$2(item.label, effectData)
          })),
          onValueChange
        };
      }
...
function buildNodes(modelsMap, preOption, parentEffect) {
  ...
  [...modelsMap].forEach(([option, model], idx) => {
    ...
    render$12 = __viewRender && (() => toNode$2(__viewRender, effectData));
    ...
    if (type === "InputGroup") {
      if (!viewRender) {
        ...
        const contents = (_b = subNodes[0].group) == null ? void 0 : _b.map(({ option: opt, content }) => {
          ...
          return () => h("span", [showLabel && toNode$2(labelSlot, effectData), showLabel && ": ", content == null ? void 0 : content()]);
        });
          ...
          return () => renderUILayout(
            ...
            { direction: breakAfter ? "vertical" : "horizontal" },
            {
              default: () => listItems.value.map(({ refData, key }, index) => {
                ...
                return h("span", { key }, [toNode$2(labelSlot2, itemEffectData), labelSlot2 ? ": " : "", refData.value]);
              })
            }
          );
        } else {
...
const _sfc_main$d = defineComponent({
  ...
  setup({ model, option, isView, effectData }, ctx) {
    ...
    slots.title || (slots.title = title && (() => toNode$2(title, effectData)));
    if (buttonsConfig) {
...
const _sfc_main$b = /* @__PURE__ */ defineComponent({
  ...
  setup(props) {
    ...
    const panes = [...props.model.children].map(([option, model], idx) => {
      ...
      const tabLabel = () => [getIconNode(icon), toNode$2(label, effectData)];
...
function createModal(content, { buttons, ...__config } = {}) {
  ...
  const titleSlot = () => config.icon ? [getIconNode(config.icon), toNode$2(config.title)] : toNode$2(config.title);
...
function editModal({ rowKey, option, listener }) {
  ...
  const getTitle = ({ meta, ...param }) => {
    return toNode$2(modalProps.title, { meta, ...param }) || `${formOption.title ? formOption.title + " - " : ""}  ${meta.title || meta.label}`;
  };
  const methods = {
    add(args = {}) {
...
function buildDetail(option, modelsMap, rowKey) {
  ...
  const getTitle = (param) => {
    return toNode$2(modalConfig.title, param) || `${title ? title + " - " : ""}详情`;
  };
...
const _sfc_main$9 = defineComponent({
  ...
  setup(props, { attrs, slots, emit }) {
    ...
    const customTab = (item) => toNode$2(innerSlots.customTab || props.customTab || item.tab, {
      ...props.effectData,
      item
    });
    return () => [
...
const _sfc_main$8 = defineComponent({
  ...
  setup({ option, model, reload, effectData, isView, ...props }, ctx) {
    ...
    __slots.headerCell = (col) => {
      var _a2;
      return ((_a2 = slots.headerCell) == null ? void 0 : _a2.call(slots, col)) || toNode$2(col.title, effectData);
    };
    const render2 = () => {
      const {
    ...
    if (option.tabs) {
      return () => h(_sfc_main$9, { ...option.tabs, effectData }, {
        ...
        title: titleSlot && (() => toNode$2(titleSlot, effectData)),
        extra: extraSlot,
        titleBar,
        default: render2
      });
...
const _sfc_main$7 = /* @__PURE__ */ defineComponent({
  ...
  setup(props, {
    ...
    }, toNode$2(title, props.effectData)), renderUIContainer("collapse", {
      ...rootAttrs,
      value: activeKey.value,
      "onUpdate:value": (value) => activeKey.value = value
    }, {
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const renderAddTag = () => renderUIPresentation(
      ...
      { default: () => [getSemanticIconNode("add"), toNode$2(props.newLabel, props.effectData)] }
    );
      ...
      h(
        ...
        {},
        {
          default: () => h(
            Button,
            defaultButtonProps,
            () => moreLabel ? toNode$1(moreLabel, effectData) : h(EllipsisOutlined)
          ),
          popupRender: () => h(
            Menu,
            {},
  ...
  return {
    ...
    services: {
      ...
      confirm(props) {
        ...
        return {
          update: (next) => modal.update(next),
          destroy: () => modal.destroy()
        };
      },
      info(props) {
        const modal = Modal.info(props);
        return {
          update: (next) => modal.update(next),
          destroy: () => modal.destroy()
        };
    ...
    modal: {
      ...
      useContext: useConfig,
      wrapContext(content, context, props) {
        var _a;
        const global2 = context == null ? void 0 : context.value;
        const rootPrefixCls = (_a = global2 == null ? void 0 : global2.getPrefixCls) == null ? void 0 : _a.call(global2);
        const prefixCls = props.prefixCls || `${rootPrefixCls}-modal`;
        return h(
          ConfigProvider,
          { ...global2, prefixCls: rootPrefixCls },
          () => content({ ...props, rootPrefixCls, prefixCls })
        );
      }
    },
    upload: {
    ...
    preview: {
      render(props) {
        ...
        return h(
          ...
          () => images.map(
            (src, index) => h(Image, { key: index, src, width, height })
          )
        );
      }
    },
        ...
        return h(
          Table,
          {
            ...
            rowSelection: selection && {
              ...
              getCheckboxProps: selection.isRowSelectable ? (row) => {
                var _a;
                return {
                  disabled: !((_a = selection.isRowSelectable) == null ? void 0 : _a.call(selection, row))
                };
              } : void 0
            },
      ...
      renderFilter(props, slots) {
        ...
        const tabs = h(
          ...
          { ...attrs, activeKey: value, "onUpdate:activeKey": onValueChange },
          {
            ...contentSlots,
            default: () => items.map(
              (item) => h(TabPane, { ...item, tab: () => item.tab })
            ),
            rightExtra: tabExtra
          }
        );
        return [tabs, (_a = slots.default) == null ? void 0 : _a.call(slots)];
      },
      selectors: {
        table: ".ant-table",
