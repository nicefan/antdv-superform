import "./style.css";
import { get, set, merge as merge$1, update, cloneDeep, isPlainObject, defaults, isArray, uniq, isFunction, mergeWith, isString, isObject, throttle, debounce, mapKeys, camelCase } from "lodash-es";
import { h, inject, reactive, ref, unref, watchEffect, computed, isRef, toValue, toRef, watch, toRefs, mergeProps, markRaw, defineComponent, openBlock, createBlock, withModifiers, withCtx, createElementBlock, Fragment, renderList, createVNode, createSlots, resolveDynamicComponent, normalizeProps, guardReactiveProps, createCommentVNode, provide, toRaw, readonly, useAttrs, onMounted, shallowReactive, getCurrentInstance, onUnmounted, nextTick, render as render$1, watchPostEffect, renderSlot, shallowRef, createTextVNode, toDisplayString, Teleport, useSlots } from "vue";
import { Tag, Modal, Space, Dropdown, Menu, MenuItem, Button, Tooltip, Divider, Card, CheckableTag, Checkbox, CheckboxGroup, Col, Collapse, CollapsePanel, DatePicker, Descriptions as Descriptions$1, DescriptionsItem, Form, FormItem, Input, InputGroup, InputNumber, InputSearch, List, ListItem, Radio, RadioButton, RadioGroup, RangePicker, Row, Select, Switch, TabPane, Table, Tabs, Textarea, TimePicker, TreeSelect, Upload, message, FormItemRest, ConfigProvider, Image } from "ant-design-vue";
import { DownOutlined, EllipsisOutlined, InfoCircleOutlined, PlusOutlined, MinusOutlined, SyncOutlined, LoadingOutlined, PaperClipOutlined, UploadOutlined, CloseCircleOutlined, UpOutlined } from "@ant-design/icons-vue";
...
function useVModel({ option, model, effectData }, defaultValue) {
  const { type, field, keepField, labelField, valueToString, computed: __computed, value, onUpdate } = option;
  const vModels = {};
  ...
  if (labelField) {
    ...
    vModels[`onUpdate:labelValue`] = (val) => {
      const value2 = valueToString ? val == null ? void 0 : val.toString() : val;
...
function buildModelData(option, origin, __chain) {
  ...
  if (refName) {
    ...
    watch(
      origin,
      (data) => {
        model.refData ?? (model.refData = toValue(value) ?? toValue(initialValue) ?? (columns && [] || subItems && {}));
        if (keepField)
          update(model.parent, keepField, (v) => v);
      },
...
function useButton(config, param, methods) {
  ...
  const allBtns = actionBtns.map((item) => {
    ...
    return { isHide: isHide2, render: render2, ...item, attrs: { ...defaultAttrs, class: _class, ...item.attrs, disabled: disabled2, onClick } };
  });
  const btns = ref([]);
  const moreBtns = ref([]);
  watchEffect(() => {
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createBlock(unref(Space), mergeProps({
        ...
        default: withCtx(() => [
          (openBlock(true), createElementBlock(Fragment, null, renderList(unref(btns), ({ attrs: attrs2, icon, label, tooltip, dropdown, menu, onClick, render: render2 }, index) => {
            return openBlock(), createElementBlock(Fragment, { key: label }, [
              dropdown ? (openBlock(), createBlock(unref(Dropdown), {
                ...
                default: withCtx(() => [
                  render2 ? (openBlock(), createBlock(resolveDynamicComponent(() => render2({ props: attrs2, ...unref(effectData) })), { key: 0 })) : (openBlock(), createBlock(unref(Button), normalizeProps(mergeProps({ key: 1 }, attrs2)), {
                    default: withCtx(() => [
                ...
                _: 2
              }, 1032, ["title"])) : (openBlock(), createElementBlock(Fragment, { key: 2 }, [
                render2 ? (openBlock(), createBlock(resolveDynamicComponent(() => render2({ props: attrs2, ...unref(effectData) })), { key: 0 })) : (openBlock(), createBlock(unref(Button), normalizeProps(mergeProps({ key: 1 }, attrs2)), {
                  default: withCtx(() => [
                    icon && !labelOnly ? (openBlock(), createBlock(resolveDynamicComponent(unref(getIconNode)(icon)), { key: 0 })) : createCommentVNode("", true),
                    (openBlock(), createBlock(resolveDynamicComponent(() => unref(toNode)(label, unref(effectData)))))
  ...
  setup(props, ctx) {
    ...
    [...props.model.children].forEach(([option, subData], idx) => {
      ...
      if (!independent && (!blocked || option.field && option.label)) {
        ...
        const rules = computed(() => unref(attrs.disabled) ? void 0 : __rules);
  ...
  [...modelsMap].forEach(([option, model], idx) => {
    ...
    if (model.children || model.listData) {
      ...
      if (type === "InputGroup") {
        ...
        if (type === "InputList") {
          if (!isBlock || __label && !(attrs == null ? void 0 : attrs.labelIndex)) {
            node = {
              option: { ...option },
              label: __label,
              hidden,
...
function useTableEdit({
  ...
  watch(() => [...orgList.value], (org) => {
    ...
    editList.value = org.map((record, idx) => {
      ...
      const hash = (_a = listItem.record)._ID_ ?? (_a._ID_ = rowKey(record) || nanoid(12));
...
function buildData({ option, model, orgList, rowKey, listener, isView, effectData }) {
  const { modelsMap: childrenMap } = model.listData;
  const context = {
    list: orgList,
    modalSlot: [],
...
const _sfc_main$d = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    if (labelField) {
      const labelName = ((_a = attrs.fieldNames) == null ? void 0 : _a.label) || "label";
      onChange = (...args) => {
        var _a2;
        const [_, item] = args;
