...
const style = "";
function useQuery(option, dataSource) {
  let isInit = false;
  nextTick(() => {
    isInit = true;
...
const _sfc_main$2 = defineComponent({
  ...
  setup(props, ctx) {
    ...
    const { loading, pagination, setPageData, onLoaded, apis, goPage, reload, query, setQueryParams, getQueryParams } = useQuery(option, dataRef);
    const exposed = {
      setOption,
      setData: (data) => {
    ...
    const unWatch = watch(
      ...
      (opt) => {
        ...
        dataRef.value ?? (dataRef.value = toRef(dataSource || []));
