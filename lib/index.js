...
import { merge, cloneDeep, mergeWith, defaults, uniq, throttle, debounce, mapKeys, camelCase } from "lodash-es";
...
function createButtons({ config, methods, params, isView }) {
  ...
  if (actions.length === 0)
    return;
  return (props = {}) => h(_sfc_main$p, { config: buttons, methods, param: params, ...props });
}
...
function buildInnerNode(option, model, effectData, attrs) {
  ...
  const { default: _, ...slots } = inject("rootSlots", {});
  ...
  [...modelsMap].forEach(([option, model], idx) => {
    ...
    if (model.children || model.listData) {
      ...
      if (type === "InputGroup") {
        ...
        const viewType = ["Tabs", "Collapse", "Card", "Table", "Group", "List"].includes(type) ? type : "Group";
...
function inlineRender({
  ...
  const editButtonsSlot = (param, config) => {
    ...
    return editInfo.isEdit ? h(_sfc_main$p, {
      key: "edit",
      config: {
        ...config,
        actions: editActions
...
function buildDetail(option, modelsMap, rowKey) {
  const source = ref({});
  const detail = () => h(_sfc_main$o, { option, modelsMap, source });
