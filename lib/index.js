...
function useBuildNode(option, model, effectData, attrs, isInner) {
  ...
  return () => h(base.FormItem, { ...formItemAttrs, name: model.propChain, rules: rules.value, colon: !!_label }, _slots);
}
const _hoisted_1$1 = {
  key: 0,
...
function useTableScroll(option, dataRef, wrapRef) {
  ...
  async function calcTableHeight() {
    ...
    if (wrapEl && inheritHeight) {
      ...
      bottomIncludeBody = tableView.bottomIncludeBody - 20;
    }
    ...
    if (isFixedHeight) {
      ...
      if (!unref(tableData) || tableData.length === 0) {
        const emptyEl = (_b = tableEl.querySelector(".ant-empty")) == null ? void 0 : _b.parentElement;
        if (emptyEl) {
