    ...
    return () => h(
      ...
      { ...formItemAttrs, rules: rules.value, ref: formItemContext, name: _propChain },
      {
        ...
        default: (slots == null ? void 0 : slots.default) || (() => h(
          FormItemRest,
          () => h(base.InputGroup, mergeProps({ compact, style: compact && { display: "inline-flex" } }, attrs), () => h(Collections, { option, model }))
        ))
      }
    );
  }
...
function useQuery(option) {
  ...
  const request = (param) => {
    ...
    loading.value = true;
    return Promise.resolve((_a = queryApi.value) == null ? void 0 : _a.call(queryApi, _params).then(setPageData)).finally(() => {
      loading.value = false;
    });
  };
...
const _sfc_main$2 = defineComponent({
  ...
  setup(props, ctx) {
    ...
    const { dataSource, loading, pagination, setPageData, onLoaded, apis, goPage, reload, query, setSearchParam } = useQuery(option);
    const exposed = {
      setOption: (_option) => {
