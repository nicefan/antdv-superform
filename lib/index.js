...
import { h, inject, reactive, ref, isRef, watchEffect, computed, toValue, toRef, watch, unref, toRefs, mergeProps, markRaw, toRaw, defineComponent, openBlock, createBlock, withModifiers, withCtx, createElementBlock, Fragment, renderList, createVNode, createSlots, resolveDynamicComponent, normalizeProps, guardReactiveProps, createCommentVNode, provide, readonly, shallowRef, useAttrs, onMounted, shallowReactive, getCurrentInstance, onUnmounted, nextTick, render as render$1, watchPostEffect, renderSlot, createTextVNode, toDisplayString, Teleport, useSlots } from "vue";
...
function updateModelIndex(model, propChain, index) {
  const previousChain = model.propChain;
  const updateModel = (current) => {
    var _a, _b;
    if (((_a = current.propChain) == null ? void 0 : _a.length) && previousChain.every((part, idx) => current.propChain[idx] === part)) {
      current.propChain = [...propChain, ...current.propChain.slice(previousChain.length)];
    }
    if (current.index !== void 0)
      current.index = index;
    (_b = current.children) == null ? void 0 : _b.forEach(updateModel);
  };
  updateModel(model);
}
...
function useRowKey(keyField) {
  ...
  const getKey = (record) => {
    ...
    const value = typeof keyField === "function" ? keyField(record) : keyField ? record[keyField] : void 0;
    const key = value !== void 0 && value !== null && value !== "" ? value : nanoid(12);
    keyMap.set(raw, key);
    return key;
  };
  const setKey = (record, key) => {
    keyMap.set(toRaw(record), key);
  };
  const hasKey = (record) => keyMap.has(toRaw(record));
...
function buildInnerNode(option, model, effectData, attrs) {
  ...
  if (type === "InfoSlot") {
    ...
    if (type === "InputSlot") {
      node = renderSlot2 && (() => renderSlot2(reactive({ props: allAttrs, ...effectData })));
    } else {
...
const _sfc_main$r = defineComponent({
  ...
  setup(props, { attrs }) {
    ...
    const isBind = model.refName !== void 0 || model.index !== void 0;
...
const _sfc_main$q = defineComponent({
  ...
  setup(props) {
    ...
    const orgModel = childrenMap.get(fristItem);
    ...
    const methods = {
      add: {
        onClick({ index, ...rest }) {
          ...
          backList.splice(index + 1, 0, void 0);
        },
        icon: () => h(PlusOutlined)
      },
      delete: {
    ...
    let rowButtonsConfig = !isView && rowButtons !== false && {
      type: "Buttons",
      buttonType: "link",
      size: "small",
      colProps: { flex: "0" },
    ...
    const genData = ({ itemOption, model: model2, idx, refData, propChain }) => {
      const ghostModel = /* @__PURE__ */ new Map([[itemOption, model2]]);
      if (labelIndex) {
        itemOption.label ?? (itemOption.label = label);
        itemOption.labelSlot ?? (itemOption.labelSlot = labelSlot || (({ index }) => itemOption.label + String(index + 1)));
      }
    ...
    if (isSingle) {
        ...
        ([list, length], [oldList = []]) => {
          ...
          listItems.value = orgList.value.map((record, idx) => {
            ...
            if (oldItem) {
              ...
              if (!isSync) {
                (_b = (_a = extProvider.formRef) == null ? void 0 : _a.value) == null ? void 0 : _b.validate([propChain]);
              }
              return oldItem;
            }
      ...
      const rowCache = /* @__PURE__ */ new WeakMap();
      watch(
        [() => [...orgList.value], () => orgList.value.length, () => [...model.propChain]],
        ([list]) => {
          if (list.length === 0) {
          ...
          listItems.value = orgList.value.map((record, idx) => {
            ...
            if (childrenMap.size === 1 && !fristItem.field && independentTypes.includes(fristItem.type)) {
              ...
              Object.assign(newModel, {
                ...
                children: cloneModels(orgModel.children || /* @__PURE__ */ new Map(), refData, propChain).modelsMap
              });
            } else {
              itemOption = {
                // ..._option,
    ...
    const render2 = () => {
      return listItems.value.map(({ model: model2, effectData: effectData2, key }) => {
        return h(Collections, { model: model2, option: { subSpan: "auto", ...option }, effectData: effectData2, key });
      });
    };
    if (isView) {
      if (isFormItem) {
      ...
      const children = computed(() => {
        return new Map(listItems.value.flatMap(({ model: model2 }) => [...model2.children]));
      });
      return () => h(DetailLayouts, {
...
const _sfc_main$o = defineComponent({
  ...
  setup({ model, option, isView, effectData }, ctx) {
    ...
    const { getKey } = useRowKey(attrs.rowKey || "id");
    const methods = {
      add() {
        orgList.value.push({});
      },
...
function useTableEdit({
  ...
  watch([() => [...orgList.value], () => [...propChain.value]], ([org]) => {
    org.forEach((record, idx) => {
      ...
      if (listItem) {
        ...
        const pathChanged = oldPropChain.length !== newPropChain.length || oldPropChain.some((part, index) => part !== newPropChain[index]);
  ...
  setup(props, ctx) {
    ...
    const fileDownload = onDownload || ((file) => {
      if (apis.download && !downloading.value) {
        ...
        apis.download({ ...file, ...reconvert(file) }).then((result) => downloadByData(result, file.name)).then(() => downModal.destroy()).catch((err) => {
