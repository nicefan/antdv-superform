...
import { merge as merge$1, cloneDeep, isPlainObject, defaults, isArray, uniq, isFunction, throttle, mergeWith, debounce, mapKeys, camelCase } from "lodash-es";
...
import { Modal, Space, Dropdown, Menu, MenuItem, Button, Tooltip, Divider, Card, CheckableTag, Checkbox, CheckboxGroup as CheckboxGroup$1, Col, Collapse, CollapsePanel, DatePicker, Descriptions as Descriptions$1, DescriptionsItem, Form, FormItem, Input, InputGroup, InputNumber, InputSearch, List, ListItem, Radio, RadioButton, RadioGroup as RadioGroup$1, RangePicker, Row, Select, Switch as Switch$1, TabPane, Table, Tabs, Tag, Textarea, TimePicker, TreeSelect, Upload, message, FormItemRest, ConfigProvider, Image } from "ant-design-vue";
...
function getViewNode(option, effectData = {}) {
  ...
  if (colType === "Upload" || colType.startsWith("Ext")) {
    ...
    return (data = effectData) => {
      ...
      return h("span", attrs, attrs.innerHTML ? void 0 : data.value);
    };
  } else {
    return content;
  }
          ...
          (openBlock(true), createElementBlock(Fragment, null, renderList(unref(btns), ({ attrs: attrs2, icon, label, tooltip, dropdown, menu, onClick }, index) => {
            return openBlock(), createElementBlock(Fragment, { key: label }, [
              dropdown ? (openBlock(), createBlock(unref(Dropdown), {
                ...
                _: 2
              }, 1032, ["disabled"])) : tooltip || unref(iconOnly) && icon ? (openBlock(), createBlock(unref(Tooltip), {
                key: 1,
                title: tooltip || label
              }, {
                default: withCtx(() => [
...
function getContent(option, model, parentEffect) {
  ...
  const effectData = toRaw(parent.value) === toRaw(parentEffect.current) ? parentEffect : getEffectData({ parent: parentEffect, current: parent, text: value, value, field: model.refName });
...
const _sfc_main$s = defineComponent({
  ...
  setup({ option, model, effectData, isView }, ctx) {
    ...
    const slots = {
      ...
      actions: buttonsSlot,
      default: ctx.slots.innerContent || (_isView ? () => h(DetailLayouts, {
        ...
        if (isSingle) {
          ...
          return () => h(
            ...
            () => listItems.value.map(({ children: children2 }) => {
              return h("span", [toNode(labelSlot2, effectData), labelSlot2 ? ": " : "", effectData.record]);
            })
          );
        } else {
...
function editModal({ initialData, rowKey, option, listener }) {
  ...
  const methods = {
    add({ meta = {}, resetData } = {}) {
      var _a;
      source.value = merge$1({}, initialData, { [rowKey]: nanoid(12), ...resetData });
...
const _sfc_main$9 = defineComponent({
  ...
  setup(props, { attrs }) {
    ...
    return () => h(
      RadioGroup,
      { name: props.option.field, optionType },
