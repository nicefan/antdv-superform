  ...
  watch(
    ...
    (data) => {
      ...
      if (refName) {
        ...
        if (keepField)
          get(model.parent, keepField) ?? set(model.parent, keepField, void 0);
      } else {
        model.refData = data;
      }
    },
...
function getViewNode(option, effectData = {}) {
  ...
  const content = (() => {
    if (labelField) {
      return ({ current } = effectData) => get(current, labelField);
    } else if (keepField) {
    ...
    return (param = effectData) => {
      ...
      const tags = arr.map((val, index) => {
        ...
        if (typeof tagOption === "function") {
          ...
          const tag = tagOption.find((item2) => item2.value === val);
          Object.assign(item, tag);
        } else {
          item.color = tagOption[val] || val === true && "success" || val === false && "error" || "default";
        }
        if (item.label === void 0 && !labels) {
          labels = content == null ? void 0 : content(param);
          labels = labels ? labels.split(",") : arr;
        }
        item.label ?? (item.label = labels[index]);
...
function editModal({ initialData, rowKey, option, listener }) {
  const source = ref({});
  const formRef = ref();
  const { modalProps: formModal, ...formOption } = option.editForm || option.formSchema || {};
...
function useColumns({ childrenMap, effectData, getEditRender, actionColumn }) {
  ...
  [...colsMap].forEach(([col, column]) => {
    ...
    const __editRender = getEditRender == null ? void 0 : getEditRender(col);
    const colEditRender = __editRender;
    if (colEditRender || viewRender) {
      const __render = (param) => {
        const result = (colEditRender == null ? void 0 : colEditRender(param)) || (viewRender == null ? void 0 : viewRender(param)) || param.text;
...
const _sfc_main$i = defineComponent({
  ...
  setup({ option, model, apis = {}, effectData, isView }, ctx) {
    ...
    const listener = {
      async onSave(data) {
        if (apis.save) {
          await apis.save({ ...data, [rowKey]: void 0 });
    ...
    if (option.indexColumn ?? ((_b = globalProps.Table) == null ? void 0 : _b.indexColumn)) {
      columns.unshift({
        dataIndex: "INDEX",
        title: "序号",
        width: 60,
        ...
        customRender: ({ index }) => {
          var _a2, _b2;
          return ((((_a2 = attrs.pagination) == null ? void 0 : _a2.current) || 1) - 1) * (((_b2 = attrs.pagination) == null ? void 0 : _b2.pageSize) || 10) + index + 1;
        },
