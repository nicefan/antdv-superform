...
import { merge, cloneDeep, mergeWith, mapKeys, camelCase } from "lodash-es";
...
const useTable = (option, data) => {
  ...
  const register = (actions, _tableRef) => {
    if (actions) {
      ...
      return (props, ctx) => h(_sfc_main$2, { dataSource: data, ...props, onRegister: register }, ctx == null ? void 0 : ctx.slots);
    }
  };
  const dataSource = computed(() => {
    var _a;
    return (_a = tableRef.value) == null ? void 0 : _a.dataRef.value;
  });
  const _promise = new Promise((resolve) => watch(tableRef, resolve));
  const getTable = () => _promise;
  ...
  return [
    register,
    {
      ...
      reload() {
        var _a;
        (_a = tableRef.value) == null ? void 0 : _a.goPage();
      },
      /** 手动执行条件查询 */
      query(param) {
        var _a;
