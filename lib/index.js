import "./style.css";
import { get, set, merge as merge$1, isNumber, update, cloneDeep, isPlainObject, defaults, uniq, isArray, throttle, isFunction, mergeWith, omit, debounce, mapKeys, camelCase } from "lodash-es";
import { h, defineComponent, inject, computed, unref, mergeProps, toRaw, reactive, ref, isRef, watchEffect, toValue, toRef, watch, toRefs, markRaw, openBlock, createBlock, resolveDynamicComponent, provide, watchPostEffect, readonly, shallowRef, useAttrs, onMounted, shallowReactive, getCurrentInstance, onUnmounted, nextTick, createVNode, render as render$1, withCtx, createElementBlock, Fragment, renderList, toDisplayString, Teleport, useSlots } from "vue";
import { useConfig } from "antdv-next/config-provider/context";
import { SpaceCompact, Form, FormItem, Tooltip, Button, Divider, Dropdown, Menu, MenuItem, Space, Card, Descriptions, DescriptionsItem, Modal, Table, Tabs, TabPane, CollapsePanel, Collapse, Input, InputNumber, InputSearch, TextArea, Select, Switch, DateRangePicker, TimeRangePicker, DatePicker, TimePicker, RadioButton, Radio, RadioGroup, Checkbox, CheckboxGroup, TreeSelect, Row, Col, Upload, Tag, CheckableTag, AutoComplete, message, ConfigProvider, Image } from "antdv-next";
import { PlusOutlined, MinusOutlined, EllipsisOutlined, DownOutlined, UpOutlined, InfoCircleOutlined, SyncOutlined, LoadingOutlined, PaperClipOutlined, UploadOutlined, CloseCircleOutlined } from "@antdv-next/icons";
...
function isBaseComponentName(name) {
            ...
            default: () => group.map(({ option, content, span, label, labelCol, wrapperCol, attrs }) => {
              ...
              return renderLayout("col", colProps, {
                default: () => renderLayout(
                  ...
                  {
                    default: () => [
                      label && renderLayout("col", mergeProps({ class: "ant-descriptions-item-label" }, labelCol), {
                      ...
                      renderLayout(
                        ...
                        {
                          default: () => !attrs.noInput && mode === "form" && label !== void 0 ? h(
                            "div",
                            {
...
function stopActionEvent(event) {
  var _a;
  const target = (event == null ? void 0 : event.domEvent) || event;
  (_a = target == null ? void 0 : target.stopPropagation) == null ? void 0 : _a.call(target);
}
function renderActionContent(button, effectData, labelOnly, iconOnly) {
  return [
    button.icon && !labelOnly ? renderAntdvIcon(button.icon, { customIcon: globalConfig.customIcon }) : void 0,
    !button.icon || !iconOnly ? toNode(button.label, effectData) : void 0
  ];
}
function renderActionButton(button, effectData, labelOnly, iconOnly) {
  var _a;
        ...
        popupRender: () => h(
          ...
          () => menu.map(
            (item) => h(
              ...
              {
                icon: item.icon ? () => renderAntdvIcon(item.icon, {
...
function renderAntdvActionGroup(props) {
  ...
  const content = buttons.flatMap((button, index) => [
    ...
    divider && index < buttons.length - 1 ? h(base.Divider, { type: "vertical", class: "sup-buttons-divider" }) : void 0
  ]);
  if (moreButtons.length) {
    content.push(
      h(
        ...
        {},
        {
          default: () => h(base.Button, defaultButtonProps, () => moreLabel ? toNode(moreLabel, effectData) : h(EllipsisOutlined)),
          popupRender: () => h(
            base.Menu,
            {},
            () => moreButtons.map(
              (button) => {
                ...
                return h(
                  ...
                  {
                    icon: button.icon ? () => renderAntdvIcon(button.icon, {
                      customIcon: globalConfig.customIcon
                    }) : void 0,
  ...
  return h(
    base.Space,
    {
      ...
      ...groupProps,
      class: ["sup-buttons", groupProps == null ? void 0 : groupProps.class]
    },
    () => content
  );
}
const antdvAdapter = {
  ...
  presentation: {
    render(type, props, slots) {
      if (type === "checkableTag") {
        ...
        return h(base.CheckableTag, { ...rest2, checked: selected, onChange: onSelectedChange }, slots);
      }
  ...
  fields: {
    Input: {
      ...
      render(component, props, _context, slots) {
        ...
        return h(
          ...
          {
            ...rest,
            enterButton: enterButtonSlot ? void 0 : enterButtonProp
          },
    ...
    RadioGroup: {
      ...
      transformProps: (props) => mapChangeEvent(props, (event) => {
        var _a;
...
function resolveUIComponent(type) {
  ...
  const component = ((_a = getUIFieldAdapter(type)) == null ? void 0 : _a.component) ?? type;
...
function resolveUILayoutComponent(type) {
...
function buildInnerNode(option, model, effectData, attrs) {
  ...
  const adapterComponent = !render2 && !definition && resolveUIComponent(type);
  const processors2 = adapterComponent ? (_a = getUIFieldAdapter(type)) == null ? void 0 : _a.processors : void 0;
  ...
  if (type === "InfoSlot") {
    ...
    if (!renderSlot) {
      ...
      } else if ((definition == null ? void 0 : definition.source) === "custom") {
        node = () => h(renderSlot, reactive(mapFormComponentModel(definition, allAttrs)), slots);
      } else if ((definition == null ? void 0 : definition.source) === "legacy" || type.startsWith("Ext")) {
...
function useModal(content, config) {
  ...
  const Wrapper = (props) => {
    ...
    return h(
      ConfigProvider,
      { ...global, prefixCls: rootPrefixCls },
...
function inlineRender({
  ...
  const InputNode2 = /* @__PURE__ */ defineComponent({
    ...
    setup({
      ...
      return () => editableRef.value ? h(base.Form, {
        ref: (instance) => {
          if (instance)
  ...
  setup({ option, model, reload, effectData, isView, ...props }, ctx) {
    ...
    const rowSelection = __rowSelection && {
      ...
      onChange: (_selectedRowKeys, _selectedRows, info) => {
        ...
        (_a2 = __rowSelection == null ? void 0 : __rowSelection.onChange) == null ? void 0 : _a2.call(__rowSelection, _selectedRowKeys, _selectedRows, info);
      },
      ...editInline && {
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  emits: ["update:value"],
  setup(__props, { emit: __emit }) {
    const Input2 = resolveUIComponent("Input");
    const props = __props;
    const emit = __emit;
    const inputRef = ref();
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        inputVisible.value ? (openBlock(), createBlock(resolveDynamicComponent(unref(Input2)), mergeProps({
          key: 0,
          ref_key: "inputRef",
          ref: inputRef
        }, inputProps.value, {
...
const controls = { ...containerComponents, ...coreFields };
const reservedSchemaTypes = /* @__PURE__ */ new Set([...Object.keys(controls), ...Object.keys(enhancedFields)]);
const definitions = Object.fromEntries([
  ...Object.entries(coreFields).map(([name, component]) => [name, { component, source: "core" }]),
  ...Object.entries(enhancedFields).map(([name, component]) => [name, { component, source: "enhanced" }])
]);
...
function addFormComponent(name, config, source = "custom") {
...
function configureComponents(components) {
  ...
  Object.entries(components).forEach(([name, config]) => {
    ...
    if (isBaseComponentName(name))
      baseOverrides[name] = component;
    if (!isBaseComponentName(name) && !reservedSchemaTypes.has(name))
      addFormComponent(name, config);
  });
  override(baseOverrides);
}
const registerFormComponents = configureComponents;
