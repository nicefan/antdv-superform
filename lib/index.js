...
import "ant-design-vue/lib/dropdown/props";
...
const Collections = defineComponent({
  ...
  setup(props, ctx) {
    ...
    const presetSpan = subSpan ?? inject("subSpan", void 0);
    if ("subSpan" in props.option)
      provide("subSpan", subSpan);
    const nodes = [];
    let currentGroup;
...
function useBuildNode(option, model, effectData, attrs, isInner) {
  const { type, label, render: render2, labelSlot } = option;
  ...
  const rules = computed(() => attrs.disabled.value ? void 0 : model.rules);
  ...
  return () => h(
    ...
    {
      label: () => (labelSlot == null ? void 0 : labelSlot(effectData)) || label,
      default: node
    }
  );
}
const _hoisted_1$1 = {
  key: 0,
  class: "exa-title ant-descriptions-header"
};
...
function useColumns({ childrenMap, effectData, getEditRender, actionColumn }) {
  ...
  [...colsMap].forEach(([col, column]) => {
    ...
    if (colEditRender || textRender) {
      const __render = (param) => h(KeepAlive, (colEditRender == null ? void 0 : colEditRender(param)) || (textRender == null ? void 0 : textRender(param)) || param.text);
...
function buildActionSlot(rowButtons, methods, getEditActions) {
  ...
  const render2 = (param) => {
    ...
    return () => actions ? h(_sfc_main$p, { key: "edit", config: { ...config, actions }, param }) : h(_sfc_main$p, { key: "view", config, param, methods });
  };
  return {
    forSlot,
    render: render2,
  ...
  [...modelsMap].forEach(([option, model], idx) => {
    ...
    if (model.children || model.listData) {
      ...
      if (["Table", "Tabs", "Collapse", "List"].includes(type)) {
        ...
        const component = viewType === "Table" ? _sfc_main$l : Controls[viewType];
...
const _sfc_main$j = /* @__PURE__ */ defineComponent({
  ...
  setup({
    ...
    return () => h(base.Card, {}, {
      ...
      extra: buttons && (() => h(_sfc_main$p, {
        config: buttons,
        param: effectData
      })),
      default: () => isView ? h(DetailLayout, {
        option,
...
const _sfc_main$i = defineComponent({
  ...
  setup({ model, option, isView, effectData }) {
    ...
    __slots.renderItem = ({ item }) => h(
      ...
      { key: item.hash },
      {
        default: () => isView ? h(DetailLayout, { option, modelsMap: item.model.children }) : [
...
function buildDetail(option, modelsMap, rowKey) {
  const source = ref({});
  const detail = () => h(_sfc_main$k, { option, modelsMap, source });
