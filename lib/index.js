...
import { merge, cloneDeep, isPlainObject, defaults, isArray, uniq, isFunction, throttle, debounce, mapKeys, camelCase } from "lodash-es";
import { h, inject, reactive, ref, unref, watchEffect, toValue, toRef, isRef, watch, computed, onMounted, toRefs, mergeProps, markRaw, defineComponent, openBlock, createBlock, withModifiers, withCtx, createElementBlock, Fragment, renderList, createVNode, createTextVNode, toDisplayString, normalizeProps, guardReactiveProps, resolveDynamicComponent, createCommentVNode, provide, toRaw, readonly, useAttrs, shallowReactive, getCurrentInstance, onUnmounted, nextTick, render as render$1, watchPostEffect, createSlots, normalizeClass, renderSlot, shallowRef, Teleport, useSlots } from "vue";
import { Modal, Space, Dropdown, Menu, MenuItem, Button, Tooltip, Divider, Card, CheckableTag, Checkbox, CheckboxGroup as CheckboxGroup$1, Col, Collapse, CollapsePanel, DatePicker, Descriptions as Descriptions$1, DescriptionsItem, Form as Form$1, FormItem, Input, InputGroup, InputNumber, InputSearch, List, ListItem, Radio, RadioButton, RadioGroup as RadioGroup$1, RangePicker, Row, Select, Switch as Switch$1, TabPane, Table, Tabs, Tag, Textarea, TimePicker, TreeSelect, Upload, message, FormItemRest, ConfigProvider, Image } from "ant-design-vue";
import { DownOutlined, EllipsisOutlined, PlusOutlined, MinusOutlined, SearchOutlined, SyncOutlined, LoadingOutlined, PaperClipOutlined, UploadOutlined, CloseCircleOutlined } from "@ant-design/icons-vue";
...
function useVModel({ option, model, effectData }, defaultValue) {
  const { type, field, keepField, computed: __computed, vModelFields, value } = option;
  if (!field)
    return;
    ...
    (data) => {
      ...
      if (refName) {
        ...
        if (columns || subItems) {
          ...
          (_b = model.parent)[refName] ?? (_b[refName] = toValue(value) ?? toValue(initialValue));
        }
...
function getViewNode(option, effectData = {}) {
  ...
  const content = (() => {
    if (labelField) {
      ...
      return ({ current, text } = effectData) => (text || "") + " - " + (current[keepField] || "");
  ...
  if (colRender || colType === "Upload" || colType.startsWith("Ext")) {
    ...
    return (data = effectData) => {
      ...
      return h("span", attrs, attrs.innerHTML ? void 0 : data.text);
    };
  } else {
    return content;
  }
        ...
        default: withCtx(() => [
          (openBlock(true), createElementBlock(Fragment, null, renderList(unref(btns), ({ attrs: attrs2, icon, label, tooltip, dropdown, menu, onClick }, index) => {
            return openBlock(), createElementBlock(Fragment, { key: label }, [
              dropdown ? (openBlock(), createBlock(unref(Dropdown), { key: 0 }, {
                overlay: withCtx(() => [
                ...
                _: 2
              }, 1024)) : tooltip || unref(iconOnly) && icon ? (openBlock(), createBlock(unref(Tooltip), {
                key: 1,
                title: tooltip || label
              }, {
                default: withCtx(() => [
                  createVNode(unref(Button), normalizeProps(guardReactiveProps(attrs2)), {
                    default: withCtx(() => [
                      icon ? (openBlock(), createBlock(resolveDynamicComponent(unref(useIcon)(icon)), { key: 0 })) : createCommentVNode("", true),
                      !icon || !unref(iconOnly) ? (openBlock(), createBlock(resolveDynamicComponent(() => unref(toNode)(label, unref(effectData))), { key: 1 })) : createCommentVNode("", true)
                    ]),
                    _: 2
                  }, 1040)
                ]),
...
const Form = /* @__PURE__ */ _export_sfc(_sfc_main$r, [["__cssModules", cssModules]]);
...
function createEditCache(childrenMap) {
  ...
  const setEditInfo = (data, info) => {
    ...
    if (!editInfo.editData) {
      ...
      const form = Form$1.useForm(editData, ref(rules));
      form.clearValidate();
...
function editModal({ initialData, rowKey, option, listener }) {
  const source = ref({});
  const formRef = ref();
  const formOption = { ...option.editForm || option.formSchema };
...
function useTableEdit({
  ...
  watch(() => [...orgList.value], (org) => {
    org.forEach((record, idx) => {
      ...
      const {
        modelsMap
      } = cloneModelsFlat(toRaw(childrenMap), toRef(listItem, "dataRef"), [...model.propChain || [], idx]);
  ...
  const InputNode = /* @__PURE__ */ defineComponent({
    ...
    setup({
      ...
      const model2 = rowMap.get(option);
      const effectData = getEffectData({
        current: toRef(model2, "parent"),
        value: toRef(model2, "refData"),
        list: orgList,
        index
      });
      const {
      ...
      return () => editableRef.value ? h(base.FormItem, reactive({
        ...
        rules,
        class: style$1["table-form-item"]
      }), inputSlot) : viewNode ? viewNode(attrs) : attrs.text;
    }
  });
  const getEditRender = (option) => {
    const component = Controls[option.type];
    if (component || option.type === "InputSlot" && option.editable !== false) {
      return (args) => h(InputNode, {
        option,
        ...args
      });
    }
...
function buildColumns(_models, colsMap = /* @__PURE__ */ new Map()) {
  ...
  [..._models].forEach(([col, model]) => {
    ...
    if (model.children) {
      ...
      const render2 = getViewNode(col);
      const column = {
        title,
        dataIndex: model.propChain.join("."),
        // ...globalProps.Column,
        ...col.columnProps,
        customRender: render2 && ((...args) => {
          const val = render2(...args);
...
function buildActionSlot({ buttons, methods, editSlot, isView }) {
  ...
  if (!buttonsSlot)
    return;
  const render2 = (param) => {
    return (editSlot == null ? void 0 : editSlot(param, config)) || buttonsSlot({ key: param.record, effectData: param });
  };
  return {
    forSlot,
    render: render2,
...
function buildDetail(option, modelsMap, rowKey) {
  ...
  const detail = () => h(_sfc_main$t, { option: _option, modelsMap, source });
...
const _sfc_main$i = defineComponent({
  ...
  setup({ option, model, apis = {}, effectData, isView }, ctx) {
    ...
    const rowKey = attrs.rowKey || "id";
    const orgList = toRef(model, "refData");
    const __rowSelection = (_a = option.attrs) == null ? void 0 : _a.rowSelection;
...
function useQuery(option, updateSource) {
  ...
  const request = (param) => {
    ...
    loading.value = true;
    return Promise.resolve((_b = queryApi.value) == null ? void 0 : _b.call(queryApi, _data).then(setPageData)).finally(() => {
      loading.value = false;
    });
  };
  const setPageData = (data) => {
    var _a;
    const res = ((_a = option.afterQuery) == null ? void 0 : _a.call(option, data)) || data;
...
function useSearchForm(tableOption, tableRef, onChange) {
  ...
  const formRef = ref();
  const dataSource = searchSchema.dataSource || reactive({});
  ...
  setup(props, ctx) {
    ...
    const unWatch = watch(
      ...
      (opt) => {
        ...
        const tabsField = (_a2 = opt.tabs) == null ? void 0 : _a2.field;
        if (tabsField) {
        ...
        tableSlot.value = () => h(Controls.Table, { option, effectData, model, ...tableAttrs }, slots.value);
      },
      {
        immediate: true
      }
