...
function inlineRender({
  ...
  const editActions = [{
    ...
    onClick: async (args) => {
      ...
      if (custom === false)
        return;
      return editInfo.form.validate().then(() => {
...
function editModal({ initialData, rowKey, option, listener }) {
  ...
  const { modalProps: formModal, ...formOption } = (rowEditor == null ? void 0 : rowEditor.form) || option.editForm || option.formSchema || {};
  ...
  const methods = {
    add(args = {}) {
      ...
      return openModal({
        ...
        onOk: async () => {
          var _a2;
          const custom = await ((_a2 = rowEditor == null ? void 0 : rowEditor.onSave) == null ? void 0 : _a2.call(rowEditor, { ...args, isNew: true }));
          if (custom === false)
            return;
    ...
    async edit(args) {
      ...
      return openModal({
        ...
        onOk: async () => {
          var _a2;
          const custom = await ((_a2 = rowEditor == null ? void 0 : rowEditor.onCancel) == null ? void 0 : _a2.call(rowEditor, { ...args, isNew: false }));
          if (custom === false)
            return;
...
const InputNode = defineComponent({
  ...
  setup({ option, effectData }) {
    ...
    const refData = computed({
      get: () => get(effectData.record, option.field),
      set: (val) => set(effectData.record, option.field, val)
    });
  ...
  [...colsMap].forEach(([col, column]) => {
    ...
    if (colEditRender || viewRender) {
      const __render = (param) => {
        const result = (colEditRender == null ? void 0 : colEditRender(param)) ?? (viewRender == null ? void 0 : viewRender(param)) ?? String(param.text ?? "");
...
function buildActionSlot({ buttons, methods, editSlot, isView, defAttrs }) {
  const buttonsConfig = {
    buttonType: "link",
    size: "small",
    ...defAttrs,
...
const _sfc_main$i = defineComponent({
  ...
  setup({ option, model, apis = {}, effectData, isView }, ctx) {
    ...
    const rowKey = (record, index) => record[attrs.rowKey] || record["_ID_"] || index;
    const orgList = toRef(model, "refData");
    ...
    if (option.indexColumn ?? ((_c = globalProps.Table) == null ? void 0 : _c.indexColumn)) {
      columns.unshift({
        dataIndex: "INDEX",
        title: "序号",
        width: 60,
