    ...
    const methods = {
      add: {
        onClick({ index }) {
          ...
          list.splice(index + 1, 0, isSingle ? void 0 : {});
          orgList.value = list;
        },
        icon: () => h(PlusOutlined)
      },
    ...
    watch(
      ...
      (currentList) => {
        ...
        const movedRows = list.map((record, idx) => {
          ...
          return !isView && rawRecord !== null && typeof rawRecord === "object" && previousItems.some((item) => item.record === rawRecord && item.index !== idx);
        });
        ...
        listItems.value = list.map((record, idx) => {
          ...
          return {
            ...
            refData,
            baseKey: keys[idx],
            key: isSingle ? `${String(keys[idx])}:${idx}:${singleVersion}` : movedRows[idx] ? `${String(keys[idx])}:${idx}` : keys[idx],
            record: rawRecord
