...
import { defaults, merge, cloneDeep, isPlainObject, isArray, uniq, throttle, isFunction, debounce, mapKeys, camelCase } from "lodash-es";
...
function getViewNode(option, effectData = {}) {
  ...
  if (colRender || colType === "Upload" || colType.startsWith("Ext")) {
    ...
    return (param = effectData) => {
      ...
      return colRender ? colRender(props) : h(Controls[colType], reactive({ option, effectData: param, ...attrs, ...vModels, isView: true }), slots);
    };
  } else {
    return content;
  }
...
function buildDetail(option, modelsMap, rowKey) {
  ...
  const detail = () => h(_sfc_main$r, { option: _option, modelsMap, source });
  ...
  setup(props, ctx) {
    ...
    const beforeUpload = (file, resFileList) => {
      ...
      if (maxCount === 1 && innerFileList.value.length) {
        ...
        info.status === "done" && ((_a = apis.delete) == null ? void 0 : _a.call(apis, outFileList.value[0]));
      }
    };
