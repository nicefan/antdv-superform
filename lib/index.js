...
function createButtons({ config, methods, params, isView }) {
  ...
  if (actions.length === 0)
    return;
  return (props = {}) => h(_sfc_main$u, { config: buttons, methods, param: params, ...props });
}
...
function buildInnerNode(option, model, effectData, attrs) {
  ...
  if (type === "Text" || type === "InfoSlot") {
    ...
    node = () => h(_sfc_main$u, { config: option, param: effectData });
...
function createModal(content, { buttons, ...__config } = {}) {
  ...
  const modalRef = ref();
  const footer = buttons && (() => h(_sfc_main$u, { config: buttons, param: { modalRef } }));
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _cache) => {
      return openBlock(true), createElementBlock(Fragment, null, renderList(unref(optionsRef), ({ label, value }) => {
        return openBlock(), createBlock(unref(CheckableTag2), {
          class: "tag-select",
          key: value,
...
const _sfc_main$1 = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    return () => h(_sfc_main$u, { config: { ...props, actions } });
  }
});
function useButtons(config) {
  const vNode = () => h(_sfc_main$u, { config });
  return [vNode];
}
