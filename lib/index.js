...
import { r as renderUIIcon, g as globalConfig, a as renderUISemanticIcon, b as renderUIPresentation, t as toNode, c as renderUIAction, d as renderUIField, e as resolveUIComponent, f as renderUILayout, h as requireUIComponent, i as getUIFieldAdapter, m as mapUIFieldProps, j as renderUIFormItem, k as renderUIContainer, l as clearUIFormValidation, n as renderUIForm, v as validateUIForm, o as registerUIComponents, p as initializeUIAdapter } from "./sdk-88f6f2dd.js";
import { q } from "./sdk-88f6f2dd.js";
import { nanoid } from "nanoid";
import { SyncOutlined, LoadingOutlined, PaperClipOutlined, PlusOutlined, UploadOutlined, CloseCircleOutlined } from "@antdv-next/icons";
...
function reportSchemaDiagnostics(schema, kind, name) {
  ...
  diagnostics.forEach(({ level, path, message: message2 }) => {
    const output = `[superform] ${path}: ${message2}`;
    if (level === "error")
...
function inlineRender({
  ...
  const InputNode2 = /* @__PURE__ */ defineComponent({
    ...
    setup({
      ...
      return () => editableRef.value ? h(baseComponents.Form, {
        ref: (instance) => {
          if (instance)
        ...
        default: () => h(baseComponents.FormItem, {
...
function useTableEdit({
  ...
  const InputNode2 = /* @__PURE__ */ defineComponent({
    ...
    setup({
      ...
      return () => selfEditableRef.value ? h(baseComponents.FormItem, reactive({
        wrapperCol: {},
...
const _sfc_main$9 = defineComponent({
  ...
  setup(props, { attrs, slots, emit }) {
    const { Card: Card2, Tabs: Tabs2, TabPane: TabPane2 } = baseComponents;
  ...
  setup(props, ctx) {
    ...
    if (listType === "picture-card") {
      ...
      slots.default = () => {
        var _a, _b;
        return [
          ((_b = (_a = ctx.slots).default) == null ? void 0 : _b.call(_a, effectData)) || h(baseComponents.Button, {}, () => [h(UploadOutlined), titleSlot ? titleSlot() : title]),
...
function registerComponents$1(definitions, components, source, additionalReservedTypes = []) {
  const reservedTypes = /* @__PURE__ */ new Set([...reservedSchemaTypes, ...additionalReservedTypes]);
...
function registerCustomComponents(components, adapterEnhancedTypes = []) {
  registerComponents$1(customDefinitions, components, "custom", [...adapterFieldTypes, ...adapterEnhancedTypes]);
}
function registerAdapterFieldTypes(types) {
  ...
  for (const name of Object.keys(customDefinitions)) {
    if (nextTypes.has(name))
      throw new Error(`Schema 类型 '${name}' 已注册为项目组件，不能再由 UIAdapter 接管`);
  }
...
function getRegisteredFormComponentTypes() {
  return [.../* @__PURE__ */ new Set([...Object.keys(customDefinitions), ...Object.keys(autoDefinitions)])];
}
  ...
  setup(props, ctx) {
    ...
    const unWatch = watch(
      ...
      (opt) => {
        ...
        if (option.editable) {
          tableSlot.value = () => h(baseComponents.Form, { model: dataRef.value, ref: tableFormRef }, table);
        } else {
