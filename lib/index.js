...
import { h, inject, reactive, toRefs, ref, unref, watchEffect, toValue, toRef, watch, onMounted, mergeProps, defineComponent, openBlock, createBlock, withModifiers, withCtx, createElementBlock, Fragment, renderList, resolveDynamicComponent, createCommentVNode, createTextVNode, toDisplayString, createVNode, normalizeProps, guardReactiveProps, provide, readonly, computed, renderSlot, useAttrs, getCurrentInstance, nextTick, render as render$1, toRaw, shallowReactive, KeepAlive, createSlots, watchPostEffect, useSlots } from "vue";
import { Modal, Space, Button, Tooltip, Dropdown, Menu, MenuItem, Card, Checkbox, CheckboxGroup, Col, Collapse, CollapsePanel, DatePicker, Descriptions as Descriptions$1, DescriptionsItem, Form as Form$1, FormItem, Input, InputGroup, InputNumber, InputSearch, List, ListItem, Radio, RadioButton, RadioGroup, RangePicker, Row, Select, Switch, TabPane, Table, Tabs, Textarea, TimePicker, TreeSelect, Divider } from "ant-design-vue";
...
function useButton(config, param, methods) {
  ...
  const allBtns = actionBtns.map((item) => {
    ...
    return { isHide: isHide2, ...item, attrs: { ...defaultAttrs, ...item.attrs, disabled: disabled2, onClick } };
  });
  const btns = ref([]);
  const moreBtns = ref([]);
  watchEffect(() => {
    ...
    if (buttons) {
      subItems = [
        ...subItems,
        {
          ...
          align: "center",
          colProps: { flex: "auto" },
          render: () => h(_sfc_main$p, { config: buttons, param: { ...effectData, formRef } })
        }
      ];
    }
...
function getContent(option, model) {
  ...
  if (viewRender || colType === "InfoSlot") {
    ...
    return () => {
      var _a, _b;
      return (_b = (_a = options.value) == null ? void 0 : _a.find(({ value: value2 }) => value2 === model.refData)) == null ? void 0 : _b.label;
    };
...
const _sfc_main$h = defineComponent({
  ...
  setup({ option, model, apis = {}, effectData, isView }, ctx) {
    ...
    const titleString = isView && ((_a = option.descriptionsProps) == null ? void 0 : _a.title) || option.label;
    ...
    if (titleString || titleSlot || extraSlot) {
      __slots.title = () => h(Row, { justify: "space-between", align: "middle", style: "width:100%" }, () => [
    ...
    const exposed = reactive({
      ...
      add: (meta) => {
        var _a2;
        return (_a2 = methods.add) == null ? void 0 : _a2.call(methods, { meta });
      },
      edit: (meta) => {
        var _a2;
        return (_a2 = methods.edit) == null ? void 0 : _a2.call(methods, { ...editParam, meta });
      },
      delete: () => {
        var _a2;
      ...
      view: (meta) => {
        var _a2;
        return (_a2 = methods.view) == null ? void 0 : _a2.call(methods, { ...editParam, meta });
      }
    });
    return () => [
...
function useSearchForm(columns, searchSechma, effectData, onChange) {
  ...
  if (actions == null ? void 0 : actions.length) {
    formOption.subItems.push({
      ...
      align: "right",
      colProps: { flex: "auto" },
      render: () => h(_sfc_main$p, { config: { ...buttonsConfig, actions }, param: effectData })
    });
  }
  ...
  setup(props, ctx) {
    ...
    const unWatch = watch(
      ...
      (opt) => {
        ...
        searchForm.value = searchSechma && useSearchForm(columns, searchSechma, { ...effectData, table }, (data) => {
          onSearch(data);
        });
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _cache) => {
      return openBlock(), createBlock(unref(_sfc_main$p), { config: _ctx.$props }, null, 8, ["config"]);
    };
  }
});
