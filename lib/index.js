...
import { defaults, merge, cloneDeep, mergeWith, uniq, throttle, debounce, mapKeys, camelCase } from "lodash-es";
...
function createButtons({ config, methods, params, isView }) {
  ...
  if (actions.length === 0)
    return;
  return (props = {}) => h(_sfc_main$q, { config: buttons, methods, param: params, ...props });
}
...
function buildInnerNode(option, model, effectData, attrs) {
  ...
  if (type === "Text" || type === "InfoSlot") {
    ...
    node = () => h(_sfc_main$q, { config: option, param: effectData });
  ...
  [...modelsMap].forEach(([option, model], idx) => {
    ...
    if (isBlock || wrapNode || idx === modelsMap.size - 1) {
      ...
      if (isBlock && !wrapNode) {
        ...
        blockNode = () => h("div", { style: style2 }, last.content());
      }
    ...
    if (wrapNode) {
      nodes.push({ option, isBlock, node: wrapNode, hidden });
    }
  });
  return nodes;
}
function getContent(option, model) {
  ...
  return content === false ? void 0 : () => {
    var _a;
    return !content ? effectData.text : typeof content === "string" ? (_a = rootSlots[content]) == null ? void 0 : _a.call(rootSlots, effectData) : content(effectData);
  };
}
...
const _sfc_main$l = defineComponent({
  ...
  setup(props, ctx) {
    ...
    if (!isContainer) {
      const innerOption = { ...option, type: "InputGroup", field: "", span: void 0 };
      innerModels = /* @__PURE__ */ new Map([[innerOption, { rules: model.rules, children: childrenMap }]]);
    }
    if (rowButtons) {
      ...
      innerModels.set(rowButtonsConfig, {});
    }
    const listItems = ref([]);
    watch(
      () => [...orgList.value],
      () => {
        ...
        listItems.value = orgList.value.map((record, idx) => {
          ...
          if (isSingle) {
            (function getIndexOpt(models, copy) {
              models.forEach((model2, opt) => {
                if (!opt.field) {
                  const __model = { ...model2, parent: orgList, refData: orgList };
                  copy.set(opt, __model);
                  if (model2.children) {
                    __model.children = /* @__PURE__ */ new Map();
                    getIndexOpt(model2.children, __model.children);
                  }
                } else if (opt.field === "index") {
                  copy.set(opt, { ...model2, parent: orgList, refData, propChain: [...propChain, idx] });
                }
              });
    ...
    ({ ...ctx.slots });
    const renderItem = () => listItems.value.map(({ model: model2, effectData: effectData2 }) => {
      return isView ? h(DetailLayouts, { option, modelsMap: model2.children }) : h(Collections, { model: model2, option, effectData: effectData2 });
    });
...
const _sfc_main$k = /* @__PURE__ */ defineComponent({
  ...
  setup({
    ...
    return () => h(base.Card, {}, {
      ...
      extra: () => buttons && !isView && h(_sfc_main$q, {
        config: buttons,
        param: effectData
      }),
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createBlock(unref(Tabs2), {
        ...
        rightExtra: withCtx(() => [!_ctx.isView && _ctx.option.buttons ? (openBlock(), createBlock(unref(_sfc_main$q), {
          key: 0,
...
function useForm(option, data) {
  ...
  const register = (actions, ref2) => {
    if (actions) {
      ...
      return (props, ctx) => h(_sfc_main$h, { ...props, onRegister: register }, ctx == null ? void 0 : ctx.slots);
    }
  };
...
function createModal(content, { buttons, ...__config } = {}) {
  ...
  const modalRef = ref();
  if (buttons) {
    __config.footer = () => h(_sfc_main$q, { config: buttons, param: { modalRef } });
  }
  const onOk = () => {
    var _a;
...
function inlineRender({
  ...
  const editButtonsSlot = (param, config) => {
    ...
    return editInfo.isEdit ? h(_sfc_main$q, {
      key: "edit",
      config: {
        ...config,
        actions: editActions
...
function buildDetail(option, modelsMap, rowKey) {
  const source = ref({});
  const detail = () => h(_sfc_main$p, { option, modelsMap, source });
              ...
              return openBlock(), createElementBlock(Fragment, { key }, [
                !hidden.value ? (openBlock(), createBlock(unref(CollapsePanel2), mergeProps({
                  ...
                  !_ctx.isView ? {
                    name: "extra",
                    fn: withCtx(() => [
                      option.buttons ? (openBlock(), createBlock(unref(_sfc_main$q), {
                        key: 0,
  ...
  setup(props, ctx) {
    ...
    const customRequest = (
      ...
      (args) => {
        ...
        apis.upload && apis.upload(formData, {
    ...
    function handleChange(info) {
      ...
      file == null ? void 0 : file.status;
      (_a = file == null ? void 0 : file.response) == null ? void 0 : _a.link;
...
const _sfc_main$1 = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    return () => h(_sfc_main$q, { config: { ...props, actions } });
  }
});
function useButtons(config) {
  const vNode = () => h(_sfc_main$q, { config });
  return [vNode];
}
