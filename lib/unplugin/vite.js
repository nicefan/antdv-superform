...
import { c as coreTypes, e as enhancedTypes } from "../schemaTypes-87eaf34d.js";
...
function filterAutoImportTypes(types, adapterEnhancedTypes = []) {
  const reservedTypes = /* @__PURE__ */ new Set([...coreTypes, ...enhancedTypes, ...adapterEnhancedTypes]);
...
function generateRuntimeModule(components, superFormImport = "antdv-superform") {
  const imports = [];
  const fields = [];
  let index = 0;
  for (const [type, result] of components) {
    ...
    fields.push(
      result.model ? `${JSON.stringify(type)}: { component: ${local}, model: ${JSON.stringify(result.model)} }` : `${JSON.stringify(type)}: ${local}`
    );
  }
  return [
...
function generateDts(components, dtsModule = "antdv-superform", typesImport = dtsModule) {
