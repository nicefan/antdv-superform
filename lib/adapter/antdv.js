...
import { b as baseComponents, g as globalConfig } from "../config-b03c8ff0.js";
...
import "antdv-next/config-provider/context";
import "antdv-next";
function renderLayout(type, props, slots) {
  return h(type === "row" ? baseComponents.Row : baseComponents.Col, props, slots);
}
...
function renderActionButton(button, effectData, labelOnly, iconOnly) {
  ...
  if (menu) {
    content = h(
      ...
      { disabled: attrs.disabled, ...button.dropdownProps },
      {
        ...
        default: () => h(baseComponents.Button, attrs, () => [
  ...
  return h(baseComponents.Tooltip, { title: unref(button.tooltipTitle) }, { default: () => content });
}
function renderAntdvActionGroup(props) {
  ...
  const content = buttons.flatMap((button, index) => [
    ...
    divider && index < buttons.length - 1 ? h(baseComponents.Divider, { type: "vertical", class: "sup-buttons-divider" }) : void 0
  ]);
  if (moreButtons.length) {
    content.push(
      h(
        ...
        {},
        {
          default: () => h(baseComponents.Button, defaultButtonProps, () => moreLabel ? toNode(moreLabel, effectData) : h(EllipsisOutlined)),
          popupRender: () => h(
            baseComponents.Menu,
            {},
            () => moreButtons.map(
...
const antdvAdapter = {
  ...
  actions: {
    render(type, props, slots) {
      return type === "group" ? renderAntdvActionGroup(props) : h(baseComponents.Tooltip, props, slots);
    }
  },
  presentation: {
    render(type, props, slots) {
      if (type === "checkableTag") {
        ...
        return h(baseComponents.CheckableTag, { ...rest2, checked: selected, onChange: onSelectedChange }, slots);
      }
      const { removable, onRemove, ...rest } = props;
      return h(baseComponents.Tag, { ...rest, closable: removable, onClose: onRemove }, slots);
    }
  },
  fields: {
    Input: {
      ...
      render(component, props, _context, slots) {
        ...
        if (!enterButtonSlot && enterButtonProp && typeof enterButtonProp === "object") {
          ...
          enterButtonSlot = () => [h(baseComponents.Button, { type: "primary", loading: searchLoading }, enterButtonProp)];
        }
        return h(
          baseComponents.InputSearch,
          {
            ...rest,
