...
适合动态上下限、占位提示、选项组件的交互属性等。它只应返回底层组件属性，不负责写模型或调用接口。属性合并顺序见[响应式与联动：dynamicAttrs](/manual/reactivity#dynamicattrs-计算底层组件属性)。
...
如果只需处理底层组件的特定交互参数，使用 `onChange`、`onSearch` 等事件。两者差异见[事件与上下文](/manual/reactivity#onupdate-与组件事件的区别)。
