...
| 动作/属性            | 说明                         |
| -------------------- | ---------------------------- |
| `openModal(config?)` | 合并配置、挂载并打开         |
| `closeModal()`       | 关闭并等待下一次 Vue tick    |
| `setModal(config)`   | 合并持久配置                 |
| `modalRef`           | 底层 Modal 引用              |
| `modalSlot`          | 手动渲染入口，主要供封装使用 |
| `formActions`        | 仅 `useModalForm` 提供       |
